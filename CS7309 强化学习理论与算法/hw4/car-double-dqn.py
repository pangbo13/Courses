...
net_A = Model(n_features, n_actions).to(device)
net_B = Model(n_features, n_actions).to(device)
...
def get_action(state, e=min_epsilon):
    if random.random() < e:
        ...
        action_A = net_A(state).softmax(dim=0)
        action_B = net_A(state).softmax(dim=0)
        action = (action_A+action_B).argmax().item()
    return action
...
def optimize_model(train_batch_size,current_episode):
    ...
    q_estimates = net_1(state.to(device)).detach()
    next_state_q_estimates = net_2(next_state.to(device)).detach()

    for i in range(len(train_sample)):
        q_estimates[i][train_sample[i][1]] = ( train_sample[i][2] +
                                              gamma * next_state_q_estimates[i].max())

    fit(net_1, state, q_estimates)


def train_one_episode(current_episode):
    global epsilon
    ...
    for _ in range(40):
        optimize_model(256,current_episode)
    epsilon -= epsilon_decay
    return step,score

def main():
    for i in range(episode_limit):
        step,score = train_one_episode(i)
