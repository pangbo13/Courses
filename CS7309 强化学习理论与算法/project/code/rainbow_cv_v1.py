...
class NormWrapper(gym.ObservationWrapper):
    def observation(self, observation):
        return observation / np.array(128.,dtype=np.float32) - 1.0
...
class Model(nn.Module):
    def __init__(self, input_features, output_values, noisy=False):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(channles, 32, kernel_size=8, stride=4, padding=2),
            nn.ReLU(),
        ...
        linear_in_features = int(
            np.prod(self.conv(torch.zeros(1, channles, w, h)).shape))

        self.flatten = nn.Flatten()
        self.fc1 = Linear(in_features=linear_in_features, out_features=512)
...
class Duel_Model(nn.Module):
    def __init__(self, input_features, output_values, noisy=False):
        ...
        self.fc1_adv = Linear(in_features=linear_in_features, out_features=512)
        self.fc2_adv = Linear(in_features=512, out_features=128)
        self.fc3_adv = Linear(in_features=128, out_features=output_values)

        self.fc1_val = Linear(in_features=linear_in_features, out_features=256)
...
def main(env_name, 
    ...
    env = TransposeWrapper(NormWrapper(env))
