...
from dataloader import load_axis_splited_data

EPOACHS = 1000

train_data,test_data = load_axis_splited_data('data/two_spiral_train_data.txt','data/two_spiral_test_data.txt')
...
TRAIN_MODEL = True
if TRAIN_MODEL:
    models = [MLQP.MLQP(2,1,[32],optimizer.SGDM,model_name=f"model_axis_minmax_{model_dataset_name[i]}") for i in range(4)]
    ...
    for i in range(len(models)):
        ...
        callback_epochs_logging.save(f"logging_axis_minmax_{model_dataset_name[i]}.mat")

    minmax_model = MinMaxMLQPPredictor(*models)
else:
    models = [MLQP.MLQP.load(f"model_axis_minmax_{model_dataset_name[i]}.npz") for i in range(4)]
    minmax_model = MinMaxMLQPPredictor(*models)
# draw boundary
DRAW_BOUNDARY = True
if DRAW_BOUNDARY:
...
if DRAW_LOGGING:
    ...
    draw_multi_logging_plot(["logging_axis_minmax_"+dname+".mat" for dname in model_dataset_name])
